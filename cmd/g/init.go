...
func InitConfig(configPath string) {
	//解析默认配置文件
	fmt.Println("Reading default config file...")
	defaultConfigFile, err := os.ReadFile("config/default_config.yaml")
	if err != nil {
		fmt.Println("Read default config failed...", err)
		return
	}
	err = yaml.Unmarshal(defaultConfigFile, &AppContext)
	if err != nil {
		fmt.Println("Resolve default config file failed!", err)
		return
	}
	fmt.Println("Default Config file read successfully!")

	//解析自定义配置文件
	fmt.Println("Reading custom config file...")
	file, err := os.ReadFile(configPath)
	if err != nil {
		fmt.Println("Read custom config failed...", err)
		return
	}
	err = yaml.Unmarshal(file, &AppContext)
	if err != nil {
		fmt.Println("Resolve custom config file failed!", err)
		return
	}
	// 将当前配置文件路径保存到环境变量中
	AppContext.ConfigFilePath = configPath
	fmt.Println("Custom config file read successfully!")
}
...
func InitSecretConfig() {
	fmt.Println("Init secret config...")
	...
	if SecToken.Discord == "" {
		log.Fatal("DISCORD_BOT_TOKEN is not set, either set it in the environment or in the config file")
	}
	if SecToken.Dify == "" {
		log.Fatal("OPENAI_API_KEY is not set, either set it in the environment or in the config file")
	}
